...
class EmbeddingExplainerTorch(object):
    ...
    def accumulation_function(
        ...
        if not second_order:
            ...
            batch_gradients = grad(
                ...
                grad_outputs=torch.ones_like(batch_predictions).to(
        ...
        batch_interpolated_alpha = (
            batch_alpha * batch_interpolated_beta + (1.0 - batch_alpha) * batch_baseline
        )
        ...
        if interaction_index is not None:
            batch_hessian = grad(
                ...
                grad_outputs=torch.ones_like(batch_gradients).to(
                    batch_predictions.device
                ),
                create_graph=True,
            )[0].detach()
        else:
            batch_hessian = torch.zeros(
                [
                    batch_input.size(0),
                    batch_input.size(1),
                    batch_input.size(1),
        ...
        if interaction_index is not None:
            ...
            batch_difference = batch_difference.unsqueeze(1).to(
                batch_predictions.device
            )
    ...
    def _single_interaction(
        ...
        current_alpha = torch.reshape(
            current_alpha, (num_samples,) + (1,) * (len(current_input.shape) - 1)
        )
        current_beta = torch.reshape(
