...
class GPT2ParallelSelfAttention(torch.nn.Module):
    ...
    def forward(self, hidden_states, ltor_mask, layer_past=None, get_present=False):
        ...
        attention_scores = torch.matmul(
            query_layer / norm_factor, key_layer.transpose(-1, -2) / norm_factor
        )
        ...
        attention_scores = torch.mul(attention_scores, ltor_mask) - 10000.0 * (
            1.0 - ltor_mask
        )
...
class GPT2ParallelMLP(torch.nn.Module):
    ...
    def __init__(
        ...
        self.dense_h_to_4h = ColumnParallelLinear(
            hidden_size, 4 * hidden_size, gather_output=False, init_method=init_method
        )
        # Project back to h.
...
class GPT2ParallelTransformerLayer(torch.nn.Module):
    ...
    def forward(self, hidden_states, ltor_mask, layer_past=None, get_present=False):
        ...
        attention_output = self.attention(
            layernorm_output, ltor_mask, layer_past=layer_past, get_present=get_present
        )
        if get_present:
...
class GPT2ParallelTransformer(torch.nn.Module):
    ...
    def forward(
        self, hidden_states, attention_mask, layer_past=None, get_present=False
    ):
...
class BertParallelTransformerLayer(torch.nn.Module):
    ...
    def __init__(
        ...
        self.intermediate = ColumnParallelLinear(
            hidden_size, intermediate_size, gather_output=False, init_method=init_method
        )
    ...
    def forward(self, hidden_states, attention_mask):
        ...
        # [b, s, h]
        attention_self_output = self.self_output(
            attention_output_parallel, hidden_states
        )
        # [b, s, ip]
