...
class EmbeddingExplainerTF(PathExplainerTF):
    ...
    def __init__(self, model, embedding_axis=2, integrate_along_axes=False, **kwargs):
        """
        ...
        self.integrate_along_axes = integrate_along_axes
        try:
    ...
    def accumulation_function(
        ...
        """
        if not second_order:
            if self.integrate_along_axes:
                input_norm = np.linalg.norm(batch_input[0])
                baseline_norm = np.linalg.norm(batch_baseline[0])
                total_norm = input_norm + baseline_norm

                batch_interpolated = np.array(
                    [
                        (1 - (total_norm / baseline_norm) * alpha) * baseline
                        if alpha < (baseline_norm / total_norm)
                        else (total_norm / input_norm)
                        * (alpha - (baseline_norm / total_norm))
                        * input_
                        for alpha, input_, baseline in zip(
                            batch_alphas, batch_input, batch_baseline
                        )
                    ]
                )
                batch_interpolated = tf.convert_to_tensor(
                    batch_interpolated, dtype=batch_input.dtype
                )
                batch_interpolated2 = np.concatenate(
                    (batch_interpolated[1:], tf.expand_dims(batch_interpolated[-1], 0)),
                    axis=0,
                )
                batch_difference = batch_interpolated2 - batch_interpolated
            else:
                batch_interpolated = (
