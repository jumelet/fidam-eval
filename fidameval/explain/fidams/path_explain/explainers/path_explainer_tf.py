...
class PathExplainerTF:
    ...
    def __init__(
        ...
        self.reuse_random_state = reuse_random_state
    ...
    def accumulation_function(
            ...
            with tf.GradientTape() as tape:
                ...
                if self.pass_original_input:
                    batch_predictions = self.model(
                        batch_interpolated, original_input=batch_input
                    )
                else:
        ...
        with tf.GradientTape() as second_order_tape:
            ...
            with tf.GradientTape() as first_order_tape:
                ...
                if self.pass_original_input:
                    batch_predictions = self.model(
                        batch_interpolated_alpha, original_input=batch_input
                    )
                else:
    ...
    def _sample_baseline(self, baseline, number_to_draw, use_expectation):
        ...
        if use_expectation:
            if self.gauss_legendre:
                num_baselines = baseline.shape[0]
                samples_per_baseline = max(1, number_to_draw // num_baselines)
                sampled_baseline = np.repeat(baseline, samples_per_baseline, axis=0)
            else:
                if self.reuse_random_state:
    ...
    def _sample_alphas(
        self, num_samples, num_baselines, use_expectation, use_product=False
    ):
        """
        ...
        """
        if use_expectation and not (use_product and self.gauss_legendre):
            if use_product:
                ...
                if self.gauss_legendre:
                    samples_per_baseline = max(1, num_samples // num_baselines)

                    alphas, weights = self._gauss_legendre(
                        samples_per_baseline, repeat=num_baselines
                    )

                    return alphas, weights
                else:
                    return (
                        np.random.uniform(low=0.0, high=1.0, size=num_samples).astype(
                            np.float32
                        ),
                        None,
                    )
            ...
            if use_product:
                ...
                if self.gauss_legendre:
                    if use_expectation:
                        samples_per_baseline = max(1, sqrt_samples // num_baselines)

                        spaced_points, weights = self._gauss_legendre(
                            samples_per_baseline, repeat=num_baselines
                        )
                    else:
                        spaced_points, weights = self._gauss_legendre(sqrt_samples)
                else:
                ...
                if weights is not None:
                    weights = np.outer(weights, weights).flatten()
                    weights = weights[slice_indices]
                    weights /= weights.sum()

                return (alpha, beta), weights
            else:
                if self.gauss_legendre:
                    alphas, weights = self._gauss_legendre(num_samples)
                else:
                    alphas = np.linspace(
    ...
    @staticmethod
    def _gauss_legendre(num_samples, repeat=1):
        alphas, weights = np.polynomial.legendre.leggauss(num_samples)
        alphas = 0.5 * (1 + alphas)

        weights *= 0.5
        # We normalize here already for the expectation over all baselines
        weights = weights / (weights.sum() * repeat)

        alphas = np.tile(alphas, repeat)
        weights = np.tile(weights, repeat)

        return alphas.astype(np.float32), weights.astype(np.float32)
    ...
    def _single_attribution(
        ...
        Args:
            ...
            current_baseline: A tensor representing the baseline input.
        ...
        if current_weights is None and self.integrate_along_axes:
            attributions = np.sum(attribution_array, axis=0)
        elif current_weights is None:
            attributions = np.mean(attribution_array, axis=0)
        else:
            attributions = (attribution_array * current_weights.reshape(-1, 1)).sum(0)
    ...
    def attributions(
        ...
        Args:
            ...
            output_indices:  If this is None, then this function returns the
                             ...
                             the input inputs[i].
        """
        if self.gauss_legendre and use_expectation:
            assert num_samples % baseline.shape[0] == 0, (
                num_samples,
                baseline.shape[0],
            )
        ...
        for i, current_input in input_iterable:
            ...
            current_alphas, current_weights = self._sample_alphas(
                num_samples, num_baselines, use_expectation
            )
    ...
    def _single_interaction(
        ...
        for j in iterator(0, num_samples, batch_size):
            number_to_draw = min(batch_size, num_samples - j)
            ...
            batch_interactions = self.accumulation_function(
                batch_input,
                batch_baseline,
            ...
            interaction_array.append(batch_interactions)
        interaction_array = np.concatenate(interaction_array, axis=0)
        if current_weights is None:
            interactions = np.mean(interaction_array, axis=0)
        else:
            interactions = (
                interaction_array * current_weights.reshape((-1, 1, 1))
            ).sum(0)
        return interactions
    ...
    def interactions(
        ...
        if self.gauss_legendre:
            ...
            ), "num_samples must be a square number"
        ...
        for i, current_input in input_iterable:
            ...
            current_alphas, current_weights = self._sample_alphas(
                num_samples, num_baselines, use_expectation, use_product=True
            )
